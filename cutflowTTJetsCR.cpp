        ...
        if ( nJets >= 2 ) { 
        	...
        	for( unsigned int iselJet = 0; iselJet < selectedJets.size(); ++iselJet ) {
        		...
        		for( unsigned int iiJet = 0; iiJet < recoJet_btag_combinedInclusiveSecondaryVertexV2BJetTags->size(); ++iiJet ){ 
        			if ( recoJet_btag_combinedInclusiveSecondaryVertexV2BJetTags->at(iJet) > 0.605 ) nBjets_CSVv2L += 1;
        			if ( recoJet_btag_combinedInclusiveSecondaryVertexV2BJetTags->at(iJet) > 0.89 ) nBjets_CSVv2M += 1;
        			if ( recoJet_btag_combinedInclusiveSecondaryVertexV2BJetTags->at(iJet) > 0.97 ) nBjets_CSVv2T += 1;
        		}
        		for( unsigned int iiJet = 0; iiJet < recoJet_btag_jetProbabilityBJetTags ->size(); ++iiJet ){ 
        			if ( recoJet_btag_jetProbabilityBJetTags ->at(iJet) > 0.275 ) nBjets_JPL += 1;
        			if ( recoJet_btag_jetProbabilityBJetTags ->at(iJet) > 0.545 ) nBjets_JPM += 1;
        			if ( recoJet_btag_jetProbabilityBJetTags ->at(iJet) > 0.790 ) nBjets_JPT += 1;
        		}
